      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            // Pick a file to study (this is optional)
            const Text("Study Material (Optional)", style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold)),
            const SizedBox(height: 12),
            Container(
              ...
              child: Column(
                children: [
                  ListTile(
                    ...
                    title: const Text("Upload New Material", style: TextStyle(color: Colors.white)),
                    onTap: () {
                      // Mock upload
                      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("File upload UI would open here")));
                    },
                  ),
                  const Divider(color: Colors.white10, height: 1),
                  if (_materials.isNotEmpty)
                    ExpansionTile(
                      title: Text(_selectedMaterial ?? "Select from Library", style: TextStyle(color: _selectedMaterial != null ? Colors.white : Colors.grey)),
                      iconColor: Colors.white,
                      collapsedIconColor: Colors.grey,
                      children: _materials.map((m) => ListTile(
                        title: Text(m, style: const TextStyle(color: Colors.white70)),
                        trailing: _selectedMaterial == m ? const Icon(Icons.check, color: Colors.greenAccent) : null,
            ...
            Container(
              ...
              decoration: BoxDecoration(
                color: AppColors.cardOverlay,
                borderRadius: BorderRadius.circular(12),
                border: Border.all(color: _companionControl ? Colors.greenAccent.withOpacity(0.3) : Colors.white10),
              ),
