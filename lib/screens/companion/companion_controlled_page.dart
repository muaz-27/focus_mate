...
class CompanionControlledPage extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<CompanionControlledPage> createState() =>
      _CompanionControlledPageState();
}

class _CompanionControlledPageState
    extends ConsumerState<CompanionControlledPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return sessionAsync.when(
      ...
      data: (sessionData) {
        ...
        if (sessionData['emergencyResponded'] != null &&
            _emergencyRequestPending) {
          WidgetsBinding.instance.addPostFrameCallback((_) {
            if (mounted)
              _showEmergencyResponseDialog(sessionData['emergencyResponded']);
          });
        }
                          ...
                          children: [
                            Row(
                              children: [
                                ...
                                Expanded(
                                  child: ElevatedButton.icon(
                                    onPressed: _emergencyRequestPending
                                        ? null
                                        : () => _requestEmergencyUnlock(
                                            "ALL_APPS",
                                          ),
                                    icon: const Icon(
                                      Icons.emergency,
