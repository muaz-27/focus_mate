...
class _StudentDashboardState extends ConsumerState<StudentDashboard>
  ...
  void initState() {
    ...
    ref.listenManual(schedulesProvider(widget.userData['id']), (previous, next) {
    ...
    _userDocSubscription = FirebaseFirestore.instance.collection('users').doc(widget.userData['id']).snapshots().listen((docSnap) {
      ...
      if (docSnap.exists) {
        ...
        final List<String> newBlockedList = List<String>.from(data['lockedApps'] ?? []);
        final DateTime? newLockEndTime = data['lockEndTime'] != null
            ? (data['lockEndTime'] as Timestamp).toDate()
            : null;
            
        if (newBlockedList.toString() != _blockedList.toString() || newLockEndTime != _lockEndTime) {
  ...
  void didUpdateWidget(StudentDashboard oldWidget) {
    ...
    debugPrint("F_MATE: didUpdateWidget -> oldSnapshotReq: $oldSnapshotRequest, newSnapshotReq: $newSnapshotRequest");
  ...
  Future<void> _applyNativeLock() async {
    ...
    if (!isAlive && mounted) {
       debugPrint("F_MATE: Native Lock attempted but Accessibility Service is OFF.");
    ...
    try {
      ...
      debugPrint("F_MATE: Synced ${appsToSend.length} user-blocked apps to native.");
    } catch (e) {
  ...
  Future<void> _linkCompanion() async {
    ...
    if (companionActive) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text("You already have a companion linked. Unlink first.")),
        );
      }
      return;
    }
    ...
    try {
      ...
      // Validate expiration
      final expiresAt = (companionData['linkCodeExpiresAt'] as Timestamp?)?.toDate();
      if (expiresAt != null && DateTime.now().isAfter(expiresAt)) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text("This code has expired. Ask your companion for a new one.")),
          );
        }
        return;
      }
      ...
      if (companionRole == 'user') {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text("Invalid code — this is not a companion account.")),
          );
        }
        return;
      }
                ...
                children: [
                   ...
                   Row(
                     ...
                     children: [
                        ...
                        Text("Limit", style: TextStyle(color: Colors.white.withValues(alpha: 0.5))),
                     ],
                   ),
                   Slider(
  ...
  Widget _buildLockModeOption({
    ...
    final borderColor = isSelected ? color : (enabled ? color.withValues(alpha: 0.3) : Colors.transparent);
    // Background color
  ...
  Widget _buildDailyFocusHero(double progress, int remaining, bool isDark) {
    ...
    List<Color> bgColors = isDark 
        ? (progress >= 1.0 
            ? [const Color(0xFF450A0A).withValues(alpha: 0.8), const Color(0xFF250505)] // Dark Red
            : progress > 0.5 
                ? [const Color(0xFF451A03).withValues(alpha: 0.8), const Color(0xFF270E02)] // Dark Orange
                : [const Color(0xFF064E3B).withValues(alpha: 0.8), const Color(0xFF022C22)]) // Dark Green
        : (progress >= 1.0 
                    ...
                    Row(
                      children: [
                        ...
                        if (_isRefreshingUsage) 
                          ...
                          InkWell(
                            ...
                            child: Padding(
                              ...
                              child: Icon(Icons.refresh, size: 16, color: accentColor.withValues(alpha: 0.8)),
                            ),
                          ),
                        const SizedBox(width: 8),
                        // Edit Button
                        InkWell(
                          ...
                          child:  Padding(
                            padding: const EdgeInsets.all(4.0),
                            child: Icon(Icons.edit, size: 16, color: accentColor.withValues(alpha: 0.8)),
                          ),
                        )
                      ],
                    ),
  ...
  Widget _buildCompanionCard(bool isDark) {
    return Container(
      ...
      decoration: BoxDecoration(
         ...
         borderRadius: BorderRadius.circular(32),
         border: Border.all(
           color: companionActive ? Colors.cyan.withValues(alpha: 0.3) : (isDark ? Colors.white.withValues(alpha: 0.08) : Colors.white),
           width: companionActive ? 1.5 : 1
         ),
         boxShadow: companionActive
            ? [BoxShadow(color: Colors.cyanAccent.withValues(alpha: 0.15), blurRadius: 24, offset: const Offset(0, 8))]
            : [BoxShadow(color: Colors.black.withValues(alpha: isDark ? 0.2 : 0.05), blurRadius: 16, offset: const Offset(0, 8))],
      ),
       child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Row(
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(
                    color: companionActive ? Colors.cyan.withValues(alpha: 0.2) : (isDark ? Colors.white.withValues(alpha: 0.05) : Colors.grey.shade200),
                 ...
                 SizedBox(
                   ...
                   child: OutlinedButton.icon(
                      ...
                      style: OutlinedButton.styleFrom(
                        side: BorderSide(color: (_activeSessionData != null) ? Colors.white10 : Colors.redAccent.withValues(alpha: 0.3)),
              ...
              Row(
                children: [
                   ...
                   Container(
                     decoration: BoxDecoration(
                       boxShadow: [
                         BoxShadow(color: Colors.cyan.withValues(alpha: 0.4), blurRadius: 12, offset: const Offset(0, 4))
                       ]
                     ),
                     child: ElevatedButton(
